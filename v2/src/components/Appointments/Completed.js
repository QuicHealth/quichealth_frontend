...
function Completed() {
  ...
  const totalCompletedAppointments = data?.Appointments.filter(
    (app) => app?.status === "passed"
  );

  if (!totalCompletedAppointments?.length)
    return <div>You have no completed appointments</div>;
  return (
    <CardsSection>
      {data?.Appointments?.filter((apps) => apps.status === "passed")?.map(
        (app, id) => {
          return (
            <Cards key={id}>
              <MobileCategory>
